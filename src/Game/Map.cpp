...
namespace sw::game
{
    ...
    bool
    Map::tryGetNextPosition(const Position currentPosition, const Position targetPosition, Position& outPosition) const
    {
    ...
    void Map::move(const uint32_t unitId, const Position position) { _positionByUnitId[unitId] = position; }

    void Map::remove(const uint32_t unitId) { _positionByUnitId.erase(unitId); }
}
