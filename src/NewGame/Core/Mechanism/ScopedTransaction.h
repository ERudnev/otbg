...
namespace swexp::core::mechanism
{
    ...
    // making deep copy of World. This can be avoided in the future (Patches or Immutability entangling, see docs for
    // details) Also it is a place for multi-threading (Transaction can share initial state with other threads and
    // submit patches to the broker) NB: to extend this, allow Transactions spawn lesser transactions and share context
    ...
    public:
        ...
        ScopedTransaction(State& target, core::ReportingContext reporting)
            : state(target),
              ...
              reporting(reporting)
        {
        } // deep copy of target state
        ~ScopedTransaction() { finish(); }

        operator Reading()
        {
            return Reading{collected, reporting};
        } // returning initial state allow independent operaions
